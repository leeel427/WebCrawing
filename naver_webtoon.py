...
myfolder = 'c:\\Users\\torsc\\Desktop\\ezen_webtoon\\'
...
try:
    ...
    pass    # 이미 만들어져 있는 경우, 오류를 무시하고 넘김

print('요일별')
'''
    save_file(): 웹 페이지에 존재하는 이미지를 로컬 컴퓨터에 저장하기 위한 함수
                 <img 태그의 src 속성>, <img src=''>는 웹 페이지에 존재하는 이미지 경로임
                 'mon', 'thu' 등 요일 정보 저장
'''
...
print('웹툰 총 개수 : %d' % (len(mytarget)))
for naver_weekday_target in mytarget:
    myhref = naver_weekday_target.find('a').attrs['href']
    ...
    imgtag = naver_weekday_target.find('img')
    #print(imgtag)
    ...
    mytuple = tuple([mytitle, myweekday, mytitle, mysrc])
